...
- `额外攻击`：当一次基础武器攻击行动结算第一击后，对同一目标再追加 `1` 次基础武器攻击；第二击独立进行命中与伤害结算，但不重选目标，也不再开启新的攻击行动。
- `动作激增`：`CD3`，立刻获得 `1` 次额外基础武器攻击行动，目标重新选择；该额外行动按完整攻击行动结算，因此可正常触发 `额外攻击`、`精准攻击`、`横扫攻击` 等相关被动。
...
- `动作激增`：主动施放日志存在。
- `额外攻击`：同一目标第二击日志存在，表现上应是一段前冲内的双碰撞后回位。
- `精准攻击`：`AC x -> y` 日志与后续 `vs AC y` 攻击检定一致，并补有短促命中特效。
- `反击战法`：登记日志先于反击出手。
- `护卫`：登记护卫反击日志先于对应受击结果。
- `横扫攻击`：基础攻击伤害、横扫追加伤害、横扫被动提示三段链路可见；表现上应是一次近战碰撞同时波及双目标。
...
- 这版可以直接作为后续职业树改造的样板，尤其可复用以下三条边界：
  - `额外攻击` 是同一行动内第二击
  - `动作激增` 是新的完整行动
  - `反击/护卫` 采用先登记后结算
