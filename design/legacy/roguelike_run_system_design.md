...
> **已过时**：职业卡三选一、招募扩编、章节 lane 节点图为主形态——见 [`character_progression_design.md`](./character_progression_design.md)（养成）与 [`dungeon_design.md`](./dungeon_design.md)（房间迷宫）。程序实现见 [`docs/implementation_guidelines.md`](../docs/implementation_guidelines.md) §5。
...
- 死亡单位不获得战斗经验。
- 升级自动完成，不占用职业卡三选一。
- 升级不触发进阶，不改变 `promotion_stage`。
- 升级若满足 `promotion_pending_target` 的等级门槛，则在经验结算内自动兑现进阶。
...
- 普通战保留金币收益，并按配置概率掉落装备。
- 精英战保留金币收益，并提供高于普通战的装备掉落期望。
- Boss 的章节奖励独立于普通战 / 精英战掉落表结算。

职业卡不进入随机掉落池，统一由指定节点结算发放。
...
| `exp_reward` | integer | 本场战斗经验 |
| `level_up_units` | string[] | 本场发生升级的单位 |
| `enter_card_reward` | boolean | 当前节点是否进入职业卡三选一 |
...
- 普通战胜利后不进入职业卡三选一
- 精英战胜利后进入职业卡三选一
- Boss 胜利后可进入章节职业卡奖励，再进入章节结算
- 招募节点提供独立职业卡三选一
- 事件节点可按结果表进入职业卡三选一
- 玩家每次只选择一张职业卡
- 职业卡是 Run 内职业阶段成长主入口
- 职业卡只处理新职业单位、同职业进阶与挂起进阶，不处理等级经验
- 职业卡不产出装备、祝福或金币
...
- `low → mid` 需要 `Lv3 + 1 张重复卡`
- `mid → high` 需要 `Lv6 + 1 张重复卡`
- 若重复卡先到、等级未达门槛，则写入 `promotion_pending_target`
- 单位升级到门槛等级时，挂起进阶自动兑现
- 当前已达高阶的职业不再进入职业卡池
...
- 已达高阶的职业不再出现
- 已有 `promotion_pending_target`、等待升级兑现的职业暂不再出现
...
| `promotion_pending_target` | enum? | 仅 `promotion_pending` 时填写目标阶段 |
| `required_level` | integer? | 仅重复卡结算时填写本段门槛等级 |
...
- 招募节点提供独立招募候选
- 招募候选优先补足当前缺位
- 招募节点与精英 / Boss 的职业卡奖励并存，承担稳定扩编与定向成长职责
