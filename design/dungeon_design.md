...
- Run 的主视觉区域始终作为“战场 / 房间舞台”使用：
  - 地图阶段显示当前房间、可用门向与探索状态；
  - 事件 / 商店 / 营地 / 奖励阶段在同一舞台上保留当前房间语境；
  - 战斗阶段切换为常规战场表现。
- 队伍状态在 Run UI 中常驻显示，玩家移动、事件选择、商店购买和战斗后结算时都能同时看到队伍 HP / 等级 / 构筑摘要。
- 迷宫不再作为独占主屏展示，只作为舞台右上角小地图，用于显示当前楼层、已探索房间、未揭示房间和当前房间位置。
- 手机竖版采用固定上下菜单 + 中间舞台结构：
  - 顶部保留现有 Run 状态 / 资源 / 控制入口；
  - 底部保留现有功能栏（地图 / 队伍 / 信息 / 日志），不被探索操作替换；
  - 上半部舞台显示房间纵深与方向选择，方向按钮直接落在地图 / 房间区域内；
  - 不显示固定的“确认 / 调查 / 遭遇”操作行；
  - 进入事件阶段时，事件按钮直接显示在事件物件旁；
  - 队伍区域与现有战斗阵容保持一致，使用 3 列前后排单位卡，而不是额外的探索队伍卡；
  - 探索阶段复用战斗场景的背景、站位计算与单位卡渲染，只在同一 canvas 上叠加探索房间、小地图和事件 / 方向按钮；
  - 地图页上半部应表现为地牢门廊 / 岔路舞台，信息只作为轻量角标，不使用大面积信息面板盖住战场；可移动方向以门 / 岔路本身作为点击目标，DOM 热区覆盖对应门位且只负责点击命中，不再绘制第二层门框、底色或方向标签。
- 事件、商店、战斗结算奖励、升级三选一等 Run 内关键选择，不再作为信息页主操作：
  - 信息页只保留摘要与回看；
  - 当前选择以 BattleScene 舞台弹窗形式覆盖在战场 / 房间舞台上；
  - 事件选择弹窗应贴近房间事件物件并保持中型尺寸，不铺满舞台，避免遮住门廊与队伍；
  - 商店购买 / 离开在舞台商店弹窗完成，不再切到信息页；商品列表可在弹窗内独立滚动，离开 / 刷新操作必须常驻在弹窗底部，不能因一屏显示不完而不可达；
  - 玩家在同一场景内完成选择后，弹窗关闭并继续回到当前房间 / 战斗结算流程。
...
- Boss 层只有 Boss 房 + 楼梯房 + 少量空房；Act1 进入 Boss 房前触发一次“终局整备”，复活 1 名阵亡队员至 `50%` 最大生命，并让存活队员恢复 `35%` 最大生命；不清冷却、不恢复大招。
- 隐藏层使用独立模板。
...
- 章节整备同时扩编出战队伍：进入第二章前补到 5 人，进入第三章前补到 6 人；扩编通过招募 reward 选择职业卡，新增队员按当前队伍等级生成并直接上场。
...
| 精英战 | 高 | 高（×1.5~2） | **必掉 1 件**（reward UI 装备卡，同宝箱） | **无** | 跟随升级 | 核心吸引力 |
...
**精英战的"质变收益包" = 装备（开箱式展示）+ 高经验**；祝福改由商店 / 事件 / Boss 等渠道获取。
