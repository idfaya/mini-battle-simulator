...
- 战斗胜利掉落为 **DMG 按 CR 的遭遇 XP**（`run_encounter_budget.lua` 数量倍率）× 章节倍率，见 `config/roguelike/battle_exp_reward.lua` / `roguelike/roguelike_run.lua`；Act1 当前使用 `chapterMultiplier = 2.30` 补偿 F4 / Boss 降压后的 CR XP 下降。
