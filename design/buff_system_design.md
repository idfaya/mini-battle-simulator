...
**燃烧规则（由上述技能链导出）**：每回合开始反射豁免（对最近 `caster` 的 `spellDC`）；成功移除，失败 `1d4` 火伤并继续；不可叠层。
...
| 技能 / 被动 | 施加 Buff | 钩子 | 联动 |
| --- | --- | --- | --- |
| `80005001` 核心被动 猎人印记 | `890005` | 远程基础攻击**命中**后（伤害可为 0） | AC / 反射各 `-value`；`IsTargetMarkedBy` |
| `80005002` 毒术精通 | `850001` 中毒 | 自身回合开始 `ProcessInfectEffect` | 已中毒目标层数 +1 |
| 狩猎指引 / 箭雨等 | — | 读取印记 | `vsMark*` 加值 |

狩猎指引命中带印记目标时，可附带 `880001` 减速（`1` 回合），见 `ranger_build_passives.lua`。
...
- **5e 口径**：持续火焰伤害；每轮 **反射豁免** 对抗施法者 **法术 DC**；成功则状态结束，失败则 `1d4` 火焰伤害。
...
| 技能链 | 5e 对齐点 | 有意简化 |
| --- | --- | --- |
| 术士燃烧 | 反射豁免 vs 法术 DC；状态可结束 | 失败全额 `1d4`；无「持续火焰」半伤档 |
...
术士：燃烧 — 反射豁免 DoT，技能 settlement 兑现
