...
| `reserve_min` | integer | `0+` | 候补最少数量 |
| `reserve_max` | integer | `0+` | 候补最多数量 |
...
| `reserve_pool_id` | string | 编号或空 | 候补候选池 |
...
| `reserve_boss_forbid` | boolean | `true` / `false` | Boss 是否禁止进入候补 |
...
| `formation_profile_id` | string | 所选编成模板 |
| `kind` | enum | 战斗类型 |
...
| `win_rule` | enum | 胜利规则 |
| `lose_rule` | enum | 失败规则 |
| `opening_enemy_ids` | string[] | 开场敌人 |
| `reserve_enemy_ids` | string[] | 候补敌人 |
| `guard_enemy_ids` | string[] | 护卫敌人 |
| `boss_enemy_id` | string | Boss 本体或空 |
...
reserve_pool_id = act1_reserve_pool_01
guard_pool_id =
boss_pool_id =
```
...
- 编成模板必须满足槽位、标签与重复数约束。
- 随机结果不得出现空战斗、空开场、无 Boss 的 Boss 战。
