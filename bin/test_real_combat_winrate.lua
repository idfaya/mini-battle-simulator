...
for seed = 1, SEED_COUNT do
    ...
    if finalSnapshot.phase == "chapter_result" and finalSnapshot.chapterResult and finalSnapshot.chapterResult.reason == "boss_defeated" then
        cleared = cleared + 1
    ...
    print(string.format("Seed %d: phase=%s reason=%s battleId=%s rounds=%s", seed, finalSnapshot.phase, reason, tostring(lastBattle.battleId), tostring(lastBattle.roundsSpent)))
end
...
print(string.format("Other Fail: %d (%.1f%%)", failed - wiped, ((failed - wiped)/SEED_COUNT)*100))
