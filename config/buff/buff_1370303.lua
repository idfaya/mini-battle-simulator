buff_1370303 = {
  ChgModelInfo = {},
  IconTexture = {
    IconPath = ""
  },
  Name = "",
  SDend = {
    duringS = 0,
    preContidion = {
      FunctionName = ""
    },
    soundid = "",
    triggerS = 0
  },
  SDstart = {
    duringS = 0,
    preContidion = {
      FunctionName = ""
    },
    soundid = "",
    triggerS = 0
  },
  SEIntervals = {
    ArenaCoordinate = {
      x = 0,
      y = 0,
      z = 0
    },
    BoneData = {
      AttachType = 0,
      BoneName = "",
      RelativePoint = 0,
      RelativePointName = "",
      fieldData = {
        placeType = 0
      },
      offset = {
        x = 0,
        y = 0,
        z = 0
      }
    },
    SfxCfgs = {},
    binding = 0,
    duringS = 0,
    effectpath = "",
    fieldData = {
      placeType = 0
    },
    preContidion = {
      FunctionName = ""
    },
    scale = {
      x = 0,
      y = 0,
      z = 0
    },
    sfxCfgpath = "",
    sfxCfgsPath = {},
    soundData = {
      duringS = 0,
      preContidion = {
        FunctionName = ""
      },
      soundid = "",
      triggerS = 0
    },
    target = 2,
    triggerS = 0
  },
  SEend = {
    ArenaCoordinate = {
      x = 0,
      y = 0,
      z = 0
    },
    BoneData = {
      AttachType = 2,
      BoneName = "",
      RelativePoint = 1,
      RelativePointName = "Cha_RP_down",
      fieldData = {
        placeType = 0
      },
      offset = {
        x = 0,
        y = 0,
        z = 0
      }
    },
    SfxCfgs = {},
    binding = 2,
    duringS = 0,
    effectpath = "Assets/Content/Prefab/SFX/FX_Hero/FX_Hero_Captain_Cold/FX_B_137_Skill_U03_03.prefab",
    fieldData = {
      placeType = 0
    },
    preContidion = {
      FunctionName = ""
    },
    scale = {
      x = 0,
      y = 0,
      z = 0
    },
    sfxCfgpath = "Assets/Res/War/Sfx/FX_B_137_Skill_U03_03.asset",
    sfxCfgsPath = {},
    soundData = {
      duringS = 0,
      preContidion = {
        FunctionName = ""
      },
      soundid = "",
      triggerS = 0
    },
    target = 2,
    triggerS = 0
  },
  SEloop = {
    ArenaCoordinate = {
      x = 0,
      y = 0,
      z = 0
    },
    BoneData = {
      AttachType = 2,
      BoneName = "",
      RelativePoint = 1,
      RelativePointName = "Cha_RP_down",
      fieldData = {
        placeType = 0
      },
      offset = {
        x = 0,
        y = 0,
        z = 0
      }
    },
    SfxCfgs = {},
    binding = 2,
    duringS = 0,
    effectpath = "Assets/Content/Prefab/SFX/FX_Hero/FX_Hero_Captain_Cold/FX_B_137_Skill_U03_02.prefab",
    fieldData = {
      placeType = 0
    },
    preContidion = {
      FunctionName = ""
    },
    scale = {
      x = 0,
      y = 0,
      z = 0
    },
    sfxCfgpath = "Assets/Res/War/Sfx/FX_B_137_Skill_U03_02.asset",
    sfxCfgsPath = {},
    soundData = {
      duringS = 0,
      preContidion = {
        FunctionName = ""
      },
      soundid = "",
      triggerS = 0
    },
    target = 2,
    triggerS = 0
  },
  SEstart = {
    ArenaCoordinate = {
      x = 0,
      y = 0,
      z = 0
    },
    BoneData = {
      AttachType = 2,
      BoneName = "",
      RelativePoint = 1,
      RelativePointName = "Cha_RP_down",
      fieldData = {
        placeType = 0
      },
      offset = {
        x = 0,
        y = 0,
        z = 0
      }
    },
    SfxCfgs = {},
    binding = 2,
    duringS = 0,
    effectpath = "Assets/Content/Prefab/SFX/FX_Hero/FX_Hero_Captain_Cold/FX_B_137_Skill_U03_01.prefab",
    fieldData = {
      placeType = 0
    },
    preContidion = {
      FunctionName = ""
    },
    scale = {
      x = 0,
      y = 0,
      z = 0
    },
    sfxCfgpath = "Assets/Res/War/Sfx/FX_B_137_Skill_U03_01.asset",
    sfxCfgsPath = {},
    soundData = {
      duringS = 0,
      preContidion = {
        FunctionName = ""
      },
      soundid = "",
      triggerS = 0
    },
    target = 2,
    triggerS = 0
  }
}