...
function HeroData.CalculateHeroAttributes(heroId, level, star, override)
    ...
    local finalHp = Ability5e.ApplyHpPacing(calculate5eHp(level, hitDie, conMod))
    local finalAc = math.max(10, math.floor(calculateArmorClass(hero.Class, dexMod, conMod, wisMod, level)))
    local finalHit = math.max(0, prof + getAttackAbilityMod(hero.Class, strMod, dexMod, intMod, wisMod) + Ability5e.GetAttackRollPacingBonus())
    local finalSpellAttack = math.max(0, prof + getSpellAbilityMod(hero.Class, intMod, wisMod, chaMod) + Ability5e.GetAttackRollPacingBonus())
    local finalSpellDC = math.max(8, 8 + prof + getSpellAbilityMod(hero.Class, intMod, wisMod, chaMod) + Ability5e.GetSpellDCPacingBonus())
